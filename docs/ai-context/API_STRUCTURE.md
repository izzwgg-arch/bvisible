...
| `finalizeEstimateAction` | `app/(app)/estimates/[id]/actions.ts` | tenant user. R-EST-04 via `evaluateEstimateFinalizeGates`: **`APPROVED`**, ≥1 linked PO, QBO on **every** linked PO, clean reconciliation. Returns `not_found`, `already_finalized`, `not_approved`, `no_linked_po`, `no_qbo_number`, `reconciliation_unresolved`, `invalid`. Status + `estimate_finalized` audit only — no pricing recompute. |
