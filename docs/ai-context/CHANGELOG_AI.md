...
## 2026-08-19 — [BID ESTIMATOR: SEVEN-STEP SIGNAGE BID WORKFLOW]

**Why:** Owner-approved frontend reference (`B-Visible-Bid-Estimator-Demo.html`) rebuilt inside the app as a real, saved, permission-aware workflow: Excel takeoff → automatic pricing from standard-sign rules → office questions → design → installation → customer-ready estimate + QBME. It extends the existing Estimates system — no second estimate product, no second pricing engine.

**What changed:**
- **Schema** (`20260819120000_bid_estimator`, additive): `EstimateType.BID`; `PricingEngine.STANDARD_SIGN` / `BID_RATE`; `estimate_line_items.qbItem` (new `QbItem` enum — the structured QuickBooks mapping); `tenant_operating_rates` gains `designHourlyCents`, `installCrewHourlyCents`, `installCrewDailyCents`, `installDayHours`, `salesTaxPercentMilli` (percent × 1000, seeded 8125 = the rate the PDF used to hardcode). New tables: `bid_estimate_workflows` (1:1 estimate — step, autosave `version`, project details, design + install inputs), `bid_source_files` (uploads + revision chain), `bid_source_rows` (EVERY parsed spreadsheet row, classified, linked to the line it feeds), `bid_line_details` (1:1 line — source qty, match, review status, explanation, AI suggestion), `bid_questions`, `standard_signs`.
- **Standard signs**: optional Google Sheet tab **`Standard Signs`** (headers in `ESTIMATE_ENGINE.md`) parsed by `lib/sheet-sync/parse-standard-signs.ts` and upserted by `lib/bid/standard-sign-sync.ts` inside `runSheetSync` — by `signKey`, duplicates skipped, vanished rows deactivated (never deleted), app-promoted rows never overwritten. Adding a sign in the Sheet needs no deploy. A missing/unrecognized tab is a no-op, never a wipe.
- **Deterministic matching + pricing** (`lib/bid/`): `parse-bid-takeoff.ts` (headings / repeated headers / subtotals / tax / totals / notes excluded and retained as evidence; same sign type combined across floors/pods), `match-standard-sign.ts` (sign key → normalized name → alias → strong fuzzy, then attribute checks → EXACT / PROBABLE / AMBIGUOUS / NONE), `price-line.ts` (per sign/set/sq ft/character/linear ft/hour/day, minimum charge, waste, rate ladder literal → Sq Ft Pricing → Meterial price × company markup, explanation steps + pricing snapshot). Billable quantity is stored separately from the source quantity (1 set → 11 characters). **AI (`lib/bid/ai-suggest.ts`) only ranks candidates with evidence + confidence for undecided lines; it never sets a quantity, size, rate, or price**, and its absence never blocks the import.
- **Office questions**: generated when the system cannot decide safely (which sign, which rate, missing size/wording, source price vs rule). Answers recalculate the line, are audited, and are project-scoped unless an admin promotes them (which adds the alias / standard sign — `promoteStandardSign`, duplicate-safe).
- **Design / installation**: recommendations from real setup work and site conditions, priced at the LIVE operating rates (design $/hr; install crew $/hr or $/day with hour equivalence), each creating a real `Design` / `Installation` estimate line with customer-facing assumptions (final electrical exclusion flows into the terms).
- **QBME rewritten** (`lib/estimate/qbme.ts`): **one line per customer estimate line, same order** (was one aggregated line per bucket). Items extended to `Channel Letters` + `Canopy` (8 allowed); ITEM comes from the stored `qbItem` (legacy lines fall back to `inferQbItem`); AMOUNT stays empty, trailing pipe kept, pipes sanitized; `reconcileQbme` proves Σ(QTY × RATE) equals the pre-tax subtotal and reports drift instead of hiding it in a large line.
- **Customer estimate / PDF**: sales tax now comes from the company setting (0 = shown pre-tax, clearly stated), terms come from `lib/estimate/estimate-terms.ts` (+ project assumptions), company identity is guarded by `lib/company/business-info.ts` (a stale Florida address / 407 phone can never print), the Item column uses the QuickBooks item, and project / PO number / status appear on the document.
- **UI**: `/estimates/new/bid` (start) and `/estimates/[id]/bid` (seven-step workspace mirroring the reference: navy step rail, guide cards, status colors, calculators, estimate sheet, QBME panel, completion checklist), debounced versioned autosave with a visible status, `/api/estimates/[id]/bid-sources/[fileId]` (tenant-gated download). Estimates list shows bid step / open questions / ready + resume link. BID estimates open **read-only** in the classic grid (its replace-all save would destroy source links and snapshots) with a link to the workflow; `saveEstimateAction` refuses them server-side.
- **Pricing backend**: Operating rates form gains the bid rates + sales tax; new read-only **Standard signs** tab showing the synced catalog and the Sheet contract when the tab is missing.

**Files:** new `apps/web/lib/bid/*`, `apps/web/app/(app)/estimates/[id]/bid/*`, `apps/web/app/(app)/estimates/new/bid/*`, `apps/web/app/api/estimates/[id]/bid-sources/[fileId]/route.ts`, `apps/web/lib/sheet-sync/parse-standard-signs.ts`, `apps/web/lib/estimate/{sales-tax,estimate-terms,recompute-estimate-totals}.ts`, `apps/web/lib/company/business-info.ts`, `packages/db/prisma/migrations/20260819120000_bid_estimator/`; modified `qbme.ts`, `estimate-pdf.ts`, `estimate-read-only-ui.ts`, `sheet-sync/{parse,sync,types}.ts`, `estimates/[id]/actions.ts`, estimates list, pricing backend, `lib/auth/audit.ts`, `schema.prisma`.

**Risks:** QBME output changes shape for EVERY estimate (line-by-line instead of bucketed) — intended, and the totals still reconcile. The tax rate moved from a code constant to a company setting seeded with the same value, so existing estimates render unchanged. Migration is additive (new nullable columns/tables + defaults); rollback = drop the six tables, the eight enum types, `estimate_line_items.qbItem` and the five `tenant_operating_rates` columns (enum VALUES cannot be dropped in place and are harmless).

**Verification:** `pnpm --filter @bvisible/web run verify:bid-estimator` (bid + QBME + presentation + sheet-sync), full `vitest run` (86 files / 644 tests), `pnpm -r typecheck`, `pnpm build`, and browser smokes `smoke:bid-estimator` (complete seven-step run incl. resume + QBME reconciliation), `smoke/estimate-classic-regression.spec.ts`, plus the pre-existing `smoke:excel-import` and assistant-takeoff smokes. Standard signs for local smokes: `tsx smoke/fixtures/seed-bid-standard-signs.ts`.
