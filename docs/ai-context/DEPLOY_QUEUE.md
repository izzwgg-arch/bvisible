...
| `deploy-once.sh <jobJson> <log>` | Fetches origin, fails if `commitHash` missing, fails if working tree dirty, checks out detached commit, snapshots release, links `shared/env/.env` and `shared/uploads`, runs `pnpm install --frozen-lockfile` + `NEXT_BUILD_STANDALONE=1 pnpm run build`. **DB phase:** `docker compose up -d db` from `$APP_DIR`, waits for `pg_isready` (≤60s), `pnpm --filter @bvisible/db exec prisma migrate deploy` (with `.env` sourced in a subshell so prisma sees `DATABASE_URL`), then `db-verify.sh`. After build/DB: copies `.next/static` into the standalone tree, writes `.bvisible-deploy-commit` (`git rev-parse HEAD`) beside standalone `server.js`, symlinks `.env` into standalone cwd, ensures `/opt/bvisible/shared/logs/pm2/` exists, runs `bash -lc 'pm2 startOrReload .../ecosystem.config.cjs --update-env'`, `bash -lc 'pm2 save --force'`, sleeps 2s, then runs `/opt/bvisible/deploy-queue/healthcheck.sh`. Migration failure → `exit 10`. db-verify failure → `exit 11`. Failed healthcheck → `exit 9`. Missing healthcheck → `exit 9`. |
