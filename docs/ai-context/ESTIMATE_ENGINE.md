...
**Estimator starting path:** `/estimates` → **New estimate** (client + job title) → `/estimates/[id]` editor. List rows expose `getEstimateListNextAction()` for the next href and `getEstimateListWorkflowChips()` for compact workflow chips; detail page uses `getEstimateEditorPrimaryAction()` for the strip CTA and `buildEstimateFinalizeChecklist()` on the fulfillment panel (display-only closeout gates — no auto-finalize). Line grid is the keyboard hub; catalog Apply and pricing helper require a focused row.
