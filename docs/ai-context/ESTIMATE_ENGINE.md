...
**Estimator starting path:** `/estimates` → **New estimate** (client + job title) → `/estimates/[id]` editor. List rows expose `getEstimateListNextAction()` for the next href; detail page uses `getEstimateEditorPrimaryAction()` for the strip CTA. Line grid is the keyboard hub; catalog Apply and pricing helper require a focused row.
