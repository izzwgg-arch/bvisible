...
- **Estimate editor** at `apps/web/app/(app)/estimates/[id]/{page,editor,line-grid,totals-panel,estimate-tools-tabs,estimate-support-tabs,vendor-catalog-intel-panel,catalog-item-picker}.tsx`:
  - **Premium estimate workspace (2026 redesign)** — compact breadcrumb/header (estimate number/name, customer, status, saved indicator, Preview/Send/Approve links) replaces the previous large hero card and retired visible workflow strip. No workflow map appears in the header. The route scopes the app shell to the mockup-like workspace: no top command bar, narrower white sidebar, and a clean light-gray canvas.
  - **Primary desktop layout** — app sidebar + main estimate workspace + sticky right summary rail. The line grid renders first and dominates the main column; setup/title/notes editing is pushed lower so it does not compete with rows. Single-column on narrow widths.
  ...
  - **`FINALIZED` read-only lockdown** — `FinalizedReadOnlyChip` on line grid + totals; static line cells (no add/delete/reorder); title/notes + multiplier/design flat display-only; catalog + pricing helper collapse without Apply; vendor intel Apply hidden; `guardedDispatch` blocks client edits; **`saveEstimateAction`** still authoritative server-side.
  - **Layout order (editor column)** — line grid first (keyboard), supporting workflow tabs second, compact estimating tools toolbar lower (Catalog / Pricing helper / Vendor intel content is collapsed until opened), setup/title/notes editor last.
  - **Line grid table** — table semantics and keyboard behavior remain unchanged; visible columns now include Type, Description, Machine, Qty, Unit, raw Cost, display-only Sell, display-only Margin, and row actions. Sell/margin are UI-only projections from existing computed line cost and multiplier; they do not mutate pricing.
  - **Supporting tabs** (`EstimateSupportTabs`) — only one secondary panel renders visibly at a time: Workflow, Files, Activity, Purchase Orders, Reconciliation, Notes. Workflow contains next step/customer quote tooling; Activity contains timeline; Purchase Orders and Reconciliation contain compact PO/recon summaries; empty tabs use compact empty states. This prevents fulfillment, quote link, response, timeline, PO, and reconciliation panels from stacking vertically.
  ...
  - **Right summary rail** (`TotalsPanel`) — sticky pricing anchor with Sell Price, Cost, Profit, Margin; collapsed pricing breakdown that also contains multiplier/design controls when not finalized; compact Customer card; compact Workflow Status list (Quote Sent, Approved, PO Created, Invoiced, Paid); then closeout, status controls, and collapsed Danger zone lower priority.
...
- **Purchase order editor** at
  ...
  - **Execution workspace** (`components/po/po-execution-workspace.tsx`): sticky operations bar at the top of `/purchase-orders/[id]` — primary/secondary CTAs (merged lifecycle + receipt actions via `lib/po/po-receipt-next-actions.ts`), compact vendor-order lifecycle chips, vendor/receipt/blocker pills, collapsible **receipt-pipeline** strip (admin; uses `getPoReceiptWorkflowSummary` + `labelOcrJobStatus` / `labelPoReconciliationStatus`), embedded **`PoLifecycleRail`** + inline **`PoLifecycleControls`**. Retired standalone **`PoOperationalRail`** and **`PoReceiptWorkflowSummaryCard`** components were removed (2026-05-25 dead-code pass).
