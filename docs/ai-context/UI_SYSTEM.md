...
- **Estimate editor** at `apps/web/app/(app)/estimates/[id]/{page,editor,line-grid,totals-panel,estimate-tools-tabs,estimate-support-tabs,vendor-catalog-intel-panel,catalog-item-picker}.tsx`:
  - **Premium estimate workspace (2026 redesign)** — compact breadcrumb/header (estimate number/name, customer, status, saved indicator, Preview/Send/Approve links) replaces the previous large hero card and retired visible workflow strip. No workflow map appears in the header.
  - **Primary desktop layout** — existing app sidebar + main estimate workspace + sticky right summary rail. The line grid renders first and dominates the main column; setup/title/notes editing is pushed lower so it does not compete with rows. Single-column on narrow widths.
  ...
  - **Layout order (editor column)** — line grid first (keyboard), compact estimating tools toolbar second (Catalog / Pricing helper / Vendor intel content is collapsed until opened), supporting workflow tabs third, setup/title/notes editor last.
