...
export function InvoiceEstimateOriginSection(props: {
  estimateId: string;
    ...
    <section className="mb-6 rounded-[var(--radius-bv)] border border-[var(--color-bv-border)] bg-[var(--color-bv-surface)] p-5 shadow-[var(--shadow-bv-card)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-[12.5px] text-[var(--color-bv-text)]">
            Linked POs on this estimate:{' '}
            <span className="font-mono font-semibold tabular-nums">{linkedPoCount}</span>
          </p>
        </div>
      ...
      <div className="mt-4 border-t border-[var(--color-bv-border)] pt-4">
        <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--color-bv-muted)]">
