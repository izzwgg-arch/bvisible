...
- **Estimate editor** at `apps/web/app/(app)/estimates/[id]/{editor,line-grid,totals-panel,vendor-catalog-intel-panel,catalog-item-picker}.tsx`:
  ...
  - **Fulfillment + lifecycle UX** — **`EstimateFulfillmentPanel`** (operational step rail, relationship strip, PO/invoice CTAs) and **`EstimateDailyWorkflowStrip`** (Draft → Sent → Approved → PO → Invoice) replace the retired standalone estimate workflow rail; closeout gates live in **`EstimateFinalizeChecklistPanel`**.
