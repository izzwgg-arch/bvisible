#!/bin/bash
# Enqueue the SMTP-mailer-foundation deploy and tail the worker until done.
# Run as deploy on the server. Mirrors .enqueue-phase4.sh.
...
cat > "$JOB" <<EOF
{
  ...
  "commitHash": "$SHA",
  "services": ["web"],
  "requestedBy": "cursor-agent-phase5",
