...
| `finalizeEstimateAction` | `app/(app)/estimates/[id]/actions.ts` | tenant user. Requires **`APPROVED`**, then seeds the PO via `createPoFromInternalMaterials` and flips status. An estimate with **no Material lines / bundle components** still finalizes — no PO is created and `message` says so (`purchaseOrderId` is undefined); only a real seeding failure blocks. Returns `not_found`, `already_finalized`, `not_approved`, `no_linked_po`, `invalid`. Status + `estimate_finalized` audit only — no pricing recompute. The stricter `evaluateEstimateFinalizeGates` (R-EST-04: QBO #, reconciliation) drives the **checklist/queue UI**, not this action. |
...
| `/dashboard` | protected | **Operations command center** — **`DashboardCommandSummary`** + **`getOperationalWorkflowQueues`** / **`DashboardOperationalQueues`** + **`getPoLifecycleDashboardQueues`** / **`DashboardPoLifecycleQueues`** (ADMIN+), **`getDashboardMetrics`** KPI chips, quick actions, vendor price alerts, collapsed **`getDashboardOperationalFeed`** recent records + audit timeline. Legacy fetch helpers **`getDashboardQuoteAttention`**, **`getDashboardEstimatePoFlow`**, **`getDashboardEstimateInvoiceFlow`** remain for Vitest (`verify:workflow-queues`, `verify:estimate-invoice-flow`) but are not mounted on the page (retired section components removed 2026-05-25). |
